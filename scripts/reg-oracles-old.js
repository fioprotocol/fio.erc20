...
const wfioContract = new ethers.Contract('0x535d2F00A72ad34b49E6Eab3500c588fCDe078f3', wfioAbi, provider);

async function main() {

    const address = await wfioContract.address;
    console.log('Address: ', address);

    try {
        registeredOracles = await wfioContract.getOracles();
        console.log('regora: ', registeredOracles);
    } catch (err) {
